...
router.get('/api/randomgambar/couplepp', cekKey, async (req, res, next) => {
	let resultt = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/kopel.json')
...
router.get('/api/randomgambar/dadu', cekKey, async (req, res, next) => {
	...
	let dadu = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/dadu.json')
...
router.get('/api/game/susunkata', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/susunkata.json')
...
router.get('/api/game/tembakbendera', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebakbendera.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/game/tembakgame', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebakgame.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/game/tembakkata', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebakkata.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/game/tembaklirik', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebaklirik.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/game/tembaklagu', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebaklagu.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/game/tembakkimia', cekKey, async (req, res, next) => {
	let ra = await fetchJson('https://raw.githubusercontent.com/bayuwonderful/data-rest-api/main/tebakkimia.json')
	let ha = ra[Math.floor(Math.random() * ra.length)]
	limitapikey(req.query.apikey)
  res.json({
	status: true,
...
router.get('/api/maker/circle', cekKey, async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'Bayu', message : "[!] cek kembali url image"}) 
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
	...
	var n = isNumber(no)
	if ( !img ) return res.json({ status : false, creator : 'Bayu', message : "[!] cek kembali url image"}) 
	if ( !n ) return res.json({ status : false, creator : 'Bayu', message : "[!] parameter no nombor sahaja"}) 
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'Bayu', message : "[!] cek kembali url image"}) 
	if ( !wid ) return res.json({ status : false, creator : 'Bayu', message : "[!] parameter width nombor sahaja"}) 
	if ( !hei ) return res.json({ status : false, creator : 'Bayu', message : "[!] parameter height nombor sahaja"}) 
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'Bayu', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Bayu', message : "[!] cek kembali url image bg"}) 
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
	...
	if ( !imgpp ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Bayu', message : "[!] masukan parameter pp Link bg dengan betul"}) 
...
router.get('api/islamic/tahlil', cekKey, async (req, res, next) => {
	fetch(encodeURI(`https://raw.githubusercontent.com/bayuslemex/islamic-rest-api-official/main/data/dataTahlil.json`)) 
